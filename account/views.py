...
class SignUpView(APIView):
    ...
    def post(self, request):
        ...
        if user_serializer.is_valid(raise_exception=True): #✏️else일 경우 400 에러 리턴하는 부분이 있어야 함.
            user = user_serializer.save() #✏️user 모델 만들기
...
class SignInView(APIView):
    ...
    def post(self, request):
        ...
        try: #✏️password가 맞는지 검증
            user = User.objects.get(username=username)
            if not user.check_password(password): #django 내부에서 check_password 등의 기능이 구현되어 있음.
                return Response(
